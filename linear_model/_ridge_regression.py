...
class RidgeRegression():
    ...
    def __init__(self, alpha=1.0, solver='sgd', batch_size=64, tolerance=1e-3, n_iter_no_change=5, learning_rate=1e-4):
        self._w = None
        self._alpha = alpha
        self._solver = solver
    ...
    def fit(self, X, y, verbose=False):
        ...
        while count_no_change < self._n_iter_no_change:
            ...
            if self._solver == 'sgd':
                ...
                loss, grad = self._cal_ridge_loss_and_grad(X[sampled, :], y[sampled], self._w, self._alpha)
            else:
                loss, grad = self._cal_ridge_loss_and_grad(X, y, self._w, self._alpha)
    ...
    @staticmethod
    def _cal_ridge_loss_and_grad(X, y, w, alpha):
        num_samples, _ = X.shape
        
        proj = np.dot(X, w)
        loss = np.sum(np.log(1 + np.exp(proj))) - np.dot(y, proj) + alpha * LA.norm(w) ** 2
        grad = np.dot(X.T, 1 - 1 / (1 + np.exp(proj)) - y) + 2 * alpha * w
